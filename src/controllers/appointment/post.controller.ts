...
const CreateAppointment = async (
  ...
  try {
    if (!req.body) {
      res
        .send({ message: "appointmnet request content cannot be empty" })
        .status(400);
      return;
    }
    ...
    if (!response) {
      res
        .send({ message: "Won't be able to create appointment!!" })
        .status(400);
      return;
    }
    ...
    res
      .send({ message: "server error while creating appointment!!" })
      .status(500);
    return;
  }
};
